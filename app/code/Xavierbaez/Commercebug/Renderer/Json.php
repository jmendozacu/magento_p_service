...
* All rights reserved
*/
namespace Xavierbaez\Commercebug\Renderer;
use Xavierbaez\Commercebug\Model\All;
use ReflectionClass;
...
class Json
{
    ...
    */    
    public function clean($data)
    {
        ...
        foreach($data as $key=>$value)
        {
            if($key === 'invoked_observers')
            {
                $data[$key] = array_filter($value, function($val){
                    return strpos($val['instance'], 'Xavierbaez\\Commercebug') === false;
                });
            }
        ...
        {
            $data['layouts'] = [
                'graph'=> \Xavierbaez\Commercebug\Plugins\MagentoFrameworkViewLayout::renderGraph(),
